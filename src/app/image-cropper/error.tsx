...
export default function ImageCropperError({ error, reset }: ErrorPageProps) {
  useEffect(() => {
    console.error("Image cropper page error:", error);
  }, [error]);
  ...
  return (
    <main className="mx-auto flex min-h-full max-w-lg flex-col items-start justify-center gap-4 px-4 py-16">
      ...
      <p className="text-sm leading-6 text-stone-600 dark:text-zinc-400">
        The cropper could not load. Try again, or refresh the page.
      </p>
      <button
        type="button"
        onClick={reset}
